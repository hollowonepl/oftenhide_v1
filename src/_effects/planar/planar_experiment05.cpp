...
//	SDL_Window *win;
	...
	virtual int on_init()
	{
		...
		std::string imagePath = getResourcePath() + "Chalkboard.png";
	...
	virtual int tunnel_test(float timestamp)
	{
		...
		for (int i=0; i<w; i++)
			for (int j=0; j<h; j++)
			{
				...
				p.x = -1+2.0*i/w+1.0;//+1.0;
				p.y = -1+2.0*j/h+2.0;//+move1.y;
				...
				uv.x = 5/(r+0.5+0.5*sin(5*a))+animation_time*0.2;
				uv.y = a*8/3.1515+animation_time*0.2;
