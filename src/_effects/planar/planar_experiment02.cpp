...
//	SDL_Window *win;
	...
	virtual int on_init()
	{
		...
		std::string imagePath = getResourcePath() + "az0!_envmap12_02.png";
	...
	virtual int tunnel_test(float timestamp)
	{
		...
		for (int i=0; i<w; i++)
			for (int j=0; j<h; j++)
			{
				...
				p.x = -1+2.0*i/w+1.0;//move1.x;
				p.y = -1+2.0*j/h+1.0;//move1.y;

				float a = atan2(p.y, p.x);
				float r = sqrt(p.dot(p))/2;

				////
				uv.x = 0.2*cos(a/3.1415)+additive_progress/8;
				uv.y = 0.5*sin(7*r)-additive_progress/8;
